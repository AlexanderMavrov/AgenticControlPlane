...
## Schema Communication to Agents

When `step_begin` prepares a step, it reads each output's struct schema file and **embeds the full schema content** inline in `outputs_text`. The agent sees the schema directly — no need to read the schema file separately. The output format (JSON, YAML, Markdown) is inferred from the file extension in the output path.
