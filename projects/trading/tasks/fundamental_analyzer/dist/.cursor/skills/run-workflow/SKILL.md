...
8. **Run token** — `run_token_text` field (**ALWAYS** append at the very end)

### 2. Spawn subagent (only when `subagent` is true)

- **Regular step**: spawn ONE subagent with the composed task
- **Parallel step** (`is_parallel: true`): read the `parallel_key` file, resolve the list, spawn ALL subagents in a SINGLE message
...
For `parallel: true` steps, manifest tracks each parallel task. On resume, only spawn subagents for `pending` and `failed` tasks — skip `completed` ones. The step's gate runs after ALL parallel tasks complete.
