...
Called by the SubagentStop hook from Claude Code (PascalCase,
.claude/settings.json) or Cursor (camelCase, .cursor/hooks.json).
Reads hook input from stdin, determines which workflow step completed,
validates outputs against struct schemas, and returns a followup_message
for the orchestrator.

Input:  JSON on stdin (SubagentStop hook payload)
...
def _is_root_relative(path):
    """Check if a path is root-relative (e.g. .agent/, .cursor/, .user/).

    Root-relative paths start with '.' followed by a letter — these are
    workspace-root anchored by convention and must NOT be prefixed with
    the workflow runtime directory.
    """
    return len(path) >= 2 and path[0] == '.' and path[1].isalpha()


def resolve_output_path(path_template, workflow_dir):
    """Resolve an output path template to actual file paths.

    Bare relative paths (data/result.json) are resolved relative to the
    **workflow directory**.  Root-relative paths (.agent/specs/...) are
    resolved from the workspace root (CWD) — they must NOT be prefixed.
    ...
    # Root-relative paths resolve from workspace root, not runtime dir
    if _is_root_relative(glob_pattern) or os.path.isabs(glob_pattern):
        abs_pattern = glob_pattern
    else:
...
def _build_trace_modifier(manifest, step_name, hook_input, gate_result,
    ...
    def modifier(trace):
        ...
        task_text = hook_input.get("task") or ""
        if existing_count == 0:
            retry_type = None
        elif loop_count > 1:
            retry_type = "gate"
        elif "<!--workflow:branch:" in task_text:
            retry_type = "branch"
        else:
            retry_type = "step"
        ...
        step_entry["retry_count"] = sum(
            1 for inv in step_entry["invocations"]
            if inv.get("retry_type") not in (None, "branch")
        )
        ...
        if gate_result.get("passed", False):
            ...
            max_gate = get_max_gate_retries(workflow_config, step_name) if workflow_config else 5
            if loop_count >= max_gate:
...
def detect_host(hook_input):
    ...
    Cursor 2.6.19+ sends 'hook_event_name' (previously CC-only), so that
    field alone is no longer a reliable discriminator.

    Strategy: check for host-specific fields first, fall back to "cursor"
    (safer default — phantom filter is harmless for CC, but skipping it
    on Cursor breaks trace and disables gate retry).
    """
    if "cursor_version" in hook_input or "loop_count" in hook_input:
        return "cursor"
    if (hook_input.get("agent_id") or hook_input.get("agent_transcript_path")
            or hook_input.get("agent_type")):
        return "claude-code"
    return "cursor"


def detect_cross_scope_writes(modified_files):
    """Detect spec writes that crossed into a scope other than the active one.

    Post-facto detection for Cursor subagentStop hook (Cursor does not
    expose a PreToolUse hook equivalent to Claude Code's, so we cannot
    block; we can only warn on the next gate iteration).

    Returns a human-readable warning string listing the offending paths,
    or None if no cross-scope writes are found.
    """
    if not modified_files:
        return None
    try:
        # Import scope_context lazily; if not deployed yet, skip silently.
    ...
    try:
        active_scope, _ = sc.resolve_active_scope()
    except Exception:
        return None

    offenders = []
    for fp in modified_files:
        path_scope = sc.scope_from_path(fp)
        if path_scope is None:
            continue  # not a spec file
        if path_scope == sc.GENERIC_SCOPE:
            continue  # generic writes are always allowed
        if path_scope == active_scope:
            continue  # match — allowed
        offenders.append((fp, path_scope))

    if not offenders:
        return None

    lines = [
        f"  - {fp} (scope: {ps})" for fp, ps in offenders
    ]
    return (
        f"SCOPE WARNING: detected {len(offenders)} spec write(s) outside "
        f"the active scope '{active_scope}'. The scope-write-validator "
        f"PreToolUse hook would have blocked these on Claude Code; under "
        f"Cursor they were committed already. Review whether this was "
        f"intentional:\n" + "\n".join(lines) + "\n"
        f"If accidental: /scope-set to the correct scope and re-run with "
        f"proper --scope param. Consider reverting the listed file(s)."
    )
...
def _output_and_exit(host, followup, passed, scope_warning=None):
    ...
    When scope_warning is non-empty and we would otherwise pass silently,
    surface the warning to the agent / user so they see the cross-scope
    writes they just made. Does NOT change the pass/fail outcome.
    """
    # Compose final followup: prepend scope warning if present
    final_followup = followup
    if scope_warning:
        if final_followup:
            final_followup = scope_warning + "\n\n" + final_followup
        else:
            # Gate passed but we still want the warning visible; emit as
            # followup_message under Cursor, or as stderr under CC.
            final_followup = scope_warning

    if host == "claude-code":
        if followup:
            # Real gate failure: block and retry
            sys.stderr.write(final_followup + "\n")
            sys.exit(2)
        else:
            # Gate passed: surface scope warning (if any) via stderr for
            # visibility, but still exit 0 so the subagent stops normally.
            if scope_warning:
                sys.stderr.write(scope_warning + "\n")
            sys.exit(0)
    else:
        # Cursor mode: JSON on stdout, always exit 0
        if final_followup:
            output = {"followup_message": final_followup}
        else:
            output = {}
...
def _main_inner():
    ...
    # ── Phantom filtering (Cursor-only) ──
    # Claude Code: matcher in settings.json filters by agent_type,
    # so only workflow-step agents reach this code.
    # Cursor: orchestrator turns also fire the hook — filter them out.
    if host == "cursor":
        ...
        # Phantom invocations: orchestrator turns have toolu_ prefix
        # or empty task field.
        subagent_id = hook_input.get("subagent_id") or ""
        task_prompt = hook_input.get("task") or ""
    ...
    # ── Find active workflow and verify via manifest ──
    # Unified for both hosts: manifest is the single source of truth
    # for run_token verification. No dependency on task prompt content.
    ...
    try:
        ...
        # Run token verification via manifest only (unified for both hosts).
        # If manifest is not in_progress, no active workflow — exit silently.
    ...
    if not step_structural:
        ...
        # ── Per-branch output isolation for parallel steps ──
        # Parallel steps have isolated _branch_N/ output dirs. Gate validates
        # each branch independently to avoid cross-branch failure cascade.
        parallel_branches = manifest.get("parallel_branches")

        if parallel_branches:
            # Determine which branch triggered this hook (if possible)
            branch_index = None
            task_text = hook_input.get("task") or ""
            branch_match = re.search(r"<!--workflow:branch:(\d+)-->", task_text)
            if branch_match:
                branch_index = int(branch_match.group(1))

            def _narrow_outputs_for_branch(outputs, bi):
                """Rewrite output paths: {variable} → _branch_N with recursive glob."""
                narrowed = []
                for out in outputs:
                    if isinstance(out, dict):
                        opath = out.get("path", "")
                    ...
                    new_parts = []
                    found_var = False
                    file_glob = parts[-1] if parts else "*"
                    for p in parts[:-1]:
                        if "{" in p:
                            if not found_var:
                                new_parts.append(f"_branch_{bi}")
                                new_parts.append("**")
                                found_var = True
                        else:
                            new_parts.append(p)
                    if not found_var:
                        new_parts.append(f"_branch_{bi}")
                        new_parts.append("**")
                    new_parts.append(file_glob)
                    n = dict(out)
                    n["path"] = "/".join(new_parts)
                    narrowed.append(n)
                return narrowed

            if branch_index is not None:
                # Known branch (Cursor): validate only this branch
                narrowed = _narrow_outputs_for_branch(outputs, branch_index)
                passed, checks, failures, details = validate_outputs(
                    narrowed, definition_dir, workflow_dir
                )
            else:
                # Unknown branch (CC: task_prompt is null).
                # Validate ALL branches independently. Collect per-branch results.
                # PASS the gate — let orchestrator handle failures via step_collect_result.
                all_branch_results = []
                total_checks = 0
                total_failures = 0
                all_details = []
                for bi_str in sorted(parallel_branches.keys()):
                    bi = int(bi_str)
                    narrowed = _narrow_outputs_for_branch(outputs, bi)
                    bp, bc, bf, bd = validate_outputs(
                        narrowed, definition_dir, workflow_dir
                    )
                    ...
                    if bd:
                        all_details.extend(
                            [f"[branch {bi}] {d}" for d in bd]
                        )
                checks = total_checks
                failures = total_failures
                details = all_details
                # Check for branches with 0 output files when outputs
                # are defined — this means the subagent wrote to the
                # wrong path, not that the branch had nothing to do.
                has_outputs_defined = any(
                    o.get("struct") for o in outputs
                )
                failed_branches = [
                    r["branch_index"] for r in all_branch_results
                    if not r["passed"]
                ]
                if has_outputs_defined and failed_branches:
                    # Gate PASS (other branches are OK) but flag failures
                    # so orchestrator can retry specific branches.
                    passed = True
                    for bi in failed_branches:
                        details.append(
                            f"[branch {bi}] EMPTY — 0 output files. "
                            f"Subagent may have written to wrong path. "
                            f"Expected: data/*/_branch_{bi}/**"
                        )
                else:
                    passed = True
                # Store per-branch results + failed list for orchestrator
                gate_result_extra = {
                    "branch_results": all_branch_results,
                }
                if failed_branches:
                    gate_result_extra["failed_branches"] = failed_branches
        else:
            # Non-parallel step: validate normally
            passed, checks, failures, details = validate_outputs(
    ...
    gate_result.update(gate_result_extra)
    result_path = os.path.join(workflow_dir, "gate-result.json")
    ...
    # Post-facto cross-scope write detection (best-effort warning).
    # Primary enforcement is scope-write-validator.py (PreToolUse hook on CC);
    # Cursor lacks an equivalent mechanism, so we inspect the transcript's
    # modified_files and surface a warning — too late to block, but at least
    # the user/agent learns about the cross-scope write on the next turn.
    scope_warning = None
    try:
        mf = hook_input.get("modified_files") or []
        if not mf:
            # Fall back to transcript extraction if hook didn't provide it
            try:
                tp = hook_input.get("agent_transcript_path")
                if tp:
                    mf = _extract_from_transcript(tp).get("modified_files", [])
            except Exception:
                mf = []
        scope_warning = detect_cross_scope_writes(mf)
    except Exception as _e:
        _log_error(f"scope-warning detection failed: {_e}")

    # Output with host-appropriate semantics
    _output_and_exit(host, followup, passed, scope_warning=scope_warning)
