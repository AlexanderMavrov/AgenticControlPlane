...
**Note:** `spec_check` adds explicit instructions to workflow subagent tasks to verify existing specs before making code changes. It is the primary mechanism for spec enforcement within workflows.
...
**Input deduplication:** When multiple input patterns resolve to the same file path with the same inject mode (e.g., `.agent/specs/{scope}/**/*.md` and `.agent/specs/generic/**/*.md` both resolve to `generic/` when `scope=generic`), the engine deduplicates automatically. Only the first occurrence is kept.

**Path resolution:** Paths starting with a dot-directory (`.user/`, `.agent/`, `.cursor/`, `.claude/`) are treated as **workspace root-relative** and are NOT prefixed with the workflow runtime directory. Only bare relative paths (like `data/report.json`) are prefixed.

## Parallel Steps

When `parallel: true`, the engine:
1. Reads `parallel_key` to resolve a list of items (from a JSON file or manifest)
2. Writes a self-contained prompt file per branch to `data/prompts/branch_N.md` (goal, inputs, outputs with schemas, constraints, run token)
3. Returns lightweight metadata to the orchestrator (file paths, subagent_type, model, summary)
4. The orchestrator spawns one subagent per branch — each reads its prompt file
5. Gate-check validates **only the branch's files** (isolated `_branch_N/` output dirs)
6. The step is complete only when ALL subagents finish and pass the gate
7. `step_complete` merges branch dirs to the final location and cleans up `_branch_N/`; prompt files persist for debugging

Each branch in the response includes a `spawn_prompt` field — a ready-to-use task prompt that the orchestrator passes directly to the Agent/Task tool. The `spawn_prompt` includes the prompt file path and a `run_token` for traceability.

**Filter normalization:** When the previous step's output contains a `[key=value]` filter field (e.g., `[status=pending]`), `step_complete` automatically normalizes all items to the expected filter value. This prevents LLM subagents from breaking parallel resolution by setting wrong values (e.g., `status: "completed"` instead of `"pending"`).
