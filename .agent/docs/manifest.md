...
| `run_id` | string | Unique run identifier (format: `<name>-<YYYYMMDD-HHmmss>-<4hex>`, e.g., `spec-enforcement-20260314-103025-a3f1`) |
| `run_token` | string | UUID for run verification. `gate-check.py` uses this to match gate results to the correct run. Appended to subagent prompts as `<!--workflow:run_token:<UUID>-->`. |
