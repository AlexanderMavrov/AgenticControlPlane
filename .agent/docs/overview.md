...
| **Spec** | A behavioral specification in `.agent/specs/{scope}/{domain}/` — requirements a component must satisfy |
| **Scope** | Context in which a spec applies (e.g., `astro`, `ares`, `generic`). Orthogonal to domain. See `specs.md` |
| **Active scope** | Currently selected working scope. Resolved per workflow run: CLI → user override → project default → `generic` |
...
**Parallel step mechanics:**
- `step_begin` writes a **self-contained prompt file** per branch (`data/prompts/branch_N.md`) and returns a slim response with `spawn_prompt` (ready-to-use task prompt including run_token).
- Gate validates each `_branch_N/` directory **independently** — no cross-branch failure cascade. In CC (where branch identity is unknown), gate passes and writes per-branch results to `gate-result.json` for the orchestrator. In Cursor (branch known from task prompt), per-branch retry goes to the correct subagent.
- `step_complete` merges `_branch_N/` dirs to the final location. Also normalizes filter fields in output data files for the next step's `parallel_key` (e.g., forces `status: "pending"`) — engine controls pipeline state, not the LLM.

**Concurrent run guard:** `workflow_init` rejects if a manifest already has `status: "in_progress"` for the same workflow. Only one active run per workflow at a time.

**Stale data cleanup:** `workflow_init` clears `data/` and `context/` from previous runs. `workflow_resume` preserves them.

## Spec Guard

Spec Guard enforces behavioral specifications during code changes. It works through four mechanisms:

1. **Spec check in workflow steps** — Workflow steps with `spec_check: true` (default) instruct the subagent to check `.agent/specs/` before modifying code. If a change would violate a spec, the subagent warns the user. This is "soft" enforcement within workflow context.
...
- `/spec-add-do EditField "requirements"` — write/update spec + implement in code (inline, ~1-2 min)
- `/spec-do EditField "instruction"` — code change with spec enforcement (inline, ~1-2 min)
- `/spec-add EditField "requirements"` — write/update spec only, no code changes (inline)
...
| `workflow_init` | Create manifest + trace + run_token. **Concurrent guard:** rejects if manifest already `in_progress`. **Stale cleanup:** deletes `data/` and `context/` from previous runs. |
| `workflow_resume` | Resume from existing manifest (preserves data/context) |
| `step_begin` | Prepare step: returns pre-composed text blocks (goal, constraints, spec_check, run_token) + `tools_info` + `model` hint. For delegation steps, returns delegation metadata. **For parallel steps:** writes self-contained prompt files to `data/prompts/branch_N.md` and returns slim metadata with `spawn_prompt` (ready-to-use task prompt including run_token). Deduplicates inputs by `(resolved_path, inject_mode)`. Respects root-relative path convention (`.user/`, `.cursor/` not prefixed with runtime dir). |
| `step_collect_result` | Read gate-result.json, determine next action (PROCEED / RETRY / STOP). **Stale detection:** compares `run_token` in gate-result.json with manifest — if mismatch, returns `stale: true` and PROCEED (orchestrator falls through to step_complete safety net). |
| `step_complete` | Mark step done, write summary, apply param_bindings, advance to next step. **Safety net:** validates outputs when gate-result.json is missing. **Parallel normalizer:** forces filter field values for next step's `parallel_key` (e.g., `status: "pending"`). |
...
**For parallel steps**, `step_begin` writes per-branch prompt files to `data/prompts/branch_N.md` inside the workflow runtime directory. Each file is self-contained: it includes the goal, branch-specific data, inlined inputs (for `inject: "file"`), output schemas, constraints, and the run token. The MCP response contains only lightweight metadata (file paths, subagent_type, model hint, and a one-line summary per branch). The orchestrator tells each subagent to read its prompt file and execute. This avoids exceeding Claude Code's MCP response token limit (default 25K tokens) when N branches × full prompt would produce very large responses. Prompt files persist after workflow completion for debugging and are referenced in the trace.
...
- `.cursor/rules/` — Always-active rules (`workflow-context.mdc`)
- `.cursor/skills/` — Skill definitions (`run-workflow`, `learn-workflows`, `spec`, `run-workflow-for-code-spec`, `spec-add-do`, `spec-do`, `spec-add`, `spec-audit`)
...
- `.claude/rules/` — Always-active rules (`workflow-context.md`)
